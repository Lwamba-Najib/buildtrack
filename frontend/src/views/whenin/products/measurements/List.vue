...
// Function to delete a measurement with confirmation
const deleteMeasurement = async (measurementId) => {
	...
	if (confirmed) {
		...
		try {
			...
			const response = await axios.delete(`/measurementdelete/${measurementId}`, {
				headers: { Authorization: `Bearer ${token}` },
			});

			if (response.data.success) {
				fetchMeasurements();
				alerts.success = "Measurement deleted successfully!";
			} else {
...
// Track selected measurements for mass delete
const selectedMeasurements = ref([]);

// Toggle measurement selection
const toggleMeasurementSelection = (measurementId) => {
	if (selectedMeasurements.value.includes(measurementId)) {
		selectedMeasurements.value = selectedMeasurements.value.filter((id) => id !== measurementId);
	} else {
		selectedMeasurements.value.push(measurementId);
	}
};

// Toggle all measurements selection
const toggleAllMeasurements = (event) => {
	if (event.target.checked) {
		selectedMeasurements.value = measurements.value.map((measurement) => measurement.id);
	} else {
...
const deleteselectedMeasurements = async () => {
	const confirmed = window.confirm(
		`Are you sure you want to delete ${selectedMeasurements.value.length} measurement(s)?`
	);
	if (confirmed && selectedMeasurements.value.length > 0) {
		alerts.success = "";
		alerts.error = "";
		isLoading.value = true;
		...
		try {
			...
			const response = await axios.delete(`/measurementsdelete`, {
				headers: { Authorization: `Bearer ${token}` },
				data: { measurement_ids: selectedMeasurements.value },
			});

			if (response.data.success) {
				fetchMeasurements();
				alerts.success = `${selectedMeasurements.value.length} measurement(s) deleted successfully!`;
				selectedMeasurements.value = [];
			} else {
				throw new Error(response.data.message);
			}
...
const fetchMeasurements = async (page = 1) => {
	...
	try {
		...
		const response = await axios.get("/measurementlist", {
			headers: {
		...
		if (response.status === 200) {
			...
			console.error("Error fetching measurement logs:", response.statusText);
		}
	} catch (error) {
		handleError(error); // Use centralized error handler for API errors
	} finally {
						...
						<div class="card-body">
							...
							<div class="position-relative">
								...
								>
									<thead>
										<tr>
											<th scope="col">
												...
												<input
													type="checkbox"
													@change="toggleAllMeasurements($event)"
												/>
											</th>
											<th scope="col">#</th>
									...
									<tbody>
										<tr v-for="(log, index) in measurements" :key="index">
											<td>
												<input
													type="checkbox"
													:value="log.id"
													@change="toggleMeasurementSelection(log.id)"
													:checked="
														selectedMeasurements.includes(log.id)
													"
												/>
											</td>
											<th scope="row">
										...
										<tr v-if="!isLoading && measurements.length === 0">
