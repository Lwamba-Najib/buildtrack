...
// Initialize reactive variables for category update fields, alerts, and loading state
const categoryId = ref(""); // Category ID for updates
const name = ref("");
...
const handleSubmit = async () => {
	...
	try {
		...
		const response = await axios.put(
			`/categoryupdate/${categoryId.value}`,
			{
				name: name.value,
			},
			{
		...
		if (response.data.success) {
			...
			alerts.error = response.data.message || "Failed to update category. Please try again.";
		}
	} catch (error) {
...
// Function to fetch the current category data
const getCategoryDetails = async () => {
	try {
		...
		const response = await axios.get(`/categoryshow/${categoryId.value}`, {
			headers: {
		...
		if (response.status === 200) {
			const category = response.data.data; // Adjust if necessary based on the actual response structure
...
onMounted(() => {
	categoryId.value = route.params.id; // Get the user ID from route params
	getCategoryDetails();
});
