...
// Function to delete a role with confirmation
const deleteRole = async (roleId) => {
	...
	if (confirmed) {
		...
		try {
			...
			const response = await axios.delete(`/roledelete/${roleId}`, {
...
// Track selected roles for mass delete
...
const deleteSelectedRoles = async () => {
	const confirmed = window.confirm(
		`Are you sure you want to delete ${selectedRoles.value.length} role(s)?`
	);
	if (confirmed && selectedRoles.value.length > 0) {
		alerts.success = "";
		alerts.error = "";
		isLoading.value = true;
		...
		try {
			...
			const response = await axios.delete(`/rolesdelete`, {
				headers: { Authorization: `Bearer ${token}` },
				data: { role_ids: selectedRoles.value },
			});

			if (response.data.success) {
				fetchRoles();
				alerts.success = `${selectedRoles.value.length} role(s) deleted successfully!`;
				selectedRoles.value = [];
			} else {
				throw new Error(response.data.message);
			}
...
const fetchRoles = async (page = 1) => {
	...
	try {
		...
		if (response.status === 200) {
			...
			console.error("Error fetching role logs:", response.statusText);
		}
	} catch (error) {
		handleError(error); // Use centralized error handler for API errors
	} finally {
						...
						<div class="card-body">
							...
							<div class="position-relative">
								...
								>
									...
									<tbody>
										<tr v-for="(log, index) in roles" :key="index">
											<td>
												<input
													type="checkbox"
													:value="log.id"
													@change="toggleRoleSelection(log.id)"
													:checked="
														selectedRoles.includes(log.id)
													"
												/>
											</td>
											<th scope="row">
