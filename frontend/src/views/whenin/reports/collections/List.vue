...
const fetchClients = async (page = 1) => {
	...
	try {
		...
		const response = await axios.get("/collectionreportclientlist", {
			headers: {
...
const exportFile = async (fileType) => {
	...
	const urls = {
		xlsx: "/collectionreportxlsx",
		csv: "/collectionreportcsv",
	};
	...
	try {
		...
		const filename = `${today}_clients.${fileType}`;
	...
	<section>
		...
		<div class="app-hero-header d-flex align-items-center">
			...
			<ol class="breadcrumb">
				...
				<li class="breadcrumb-item">
					<RouterLink to="/collectionreportlist" class="text-decoration-none"
						>Collection Transfers Reports</RouterLink
					>
				</li>
		...
		<div class="app-body">
			...
			<div
				class="row"
				v-if="
					menuAccess.collectionReportListExport ||
					menuAccess.collectionReportListMaskToggle
				"
			>
						...
						<div class="card-body p-2">
							<div class="d-flex justify-content-end my-1 my-lg-0">
								<div class="d-flex flex-row gap-2">
									<button v-if="menuAccess.collectionReportListMaskToggle" @click="toggleMask" class="btn btn-sm btn-info d-flex align-items-center gap-2">
                                    <i :class="isMasked ? 'bi-eye-slash' : 'bi-eye'"></i>
                                    {{ isMasked ? "Unmask" : "Mask" }}
                                    </button>
									<div class="d-flex" v-if="menuAccess.collectionReportListExport">
										<div class="dropdown">
											<button
												type="button"
								...
								>
									...
									<tbody>
										<tr v-for="(log, index) in clients" :key="index">
											...
											<td class="text-end">{{ maskValue(Number(log.total_credit || 0).toLocaleString(), 'debit') }}</td>
