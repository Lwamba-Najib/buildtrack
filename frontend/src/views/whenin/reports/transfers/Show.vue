...
const fetchDetails = async () => {
    ...
    try {
        ...
        // Fetch the specific transfer report by batch number
        const response = await axios.get(`/transferreportshow/${route.params.id}`, {
            headers: {
        ...
        if (response.status === 200 && response.data.success) {
            ...
            console.error('Error fetching transfer report:', response.statusText);
        }
    } catch (error) {
        handleError(error); // Handle error using centralized error handle
        transferreport.value = {};
        transactions.value = [];
    } finally {
...
const fetchTotalDebits = async (mno = null) => {
    isLoading.value = true; // Start loading

    try {
        const token = getToken(); // Retrieve the token
        
        let url = `/transferreporttotaldebit/${route.params.id}`;  // No need to include clientId in the URL
        if (mno) {
            url += `/${mno}`;
        }
        ...
        if (response.status === 200 && response.data.success) {
            ...
            alerts.error = response.data.message || "Failed to load transfer totals.";
        }
    } catch (error) {
        handleError(error);
    } finally {
...
const exportPDF = async () => {
    ...
    try {
        ...
        const response = await axios.get(`/transferreportshowpdf/${route.params.id}`, {
            headers: {
        ...
        const filename = `${today}_transferreportshow_${id}.pdf`;
    ...
    <section>
        ...
        <div class="app-hero-header d-flex align-items-center">
            ...
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item">
                    <RouterLink to="/transferreportlist" class="text-decoration-none">Transfers Report</RouterLink>
                </li>
        ...
        <div class="app-body">
            ...
            <div class="row" 
                v-if="
					menuAccess.transferReportShowMaskToggle ||
					menuAccess.transferReportShowExport
				">
                        ...
                        <div class="card-body p-2">
                            <div class="d-flex justify-content-end my-1 my-lg-0">
                                <div class="d-flex flex-row gap-2">
                                    <button v-if="menuAccess.transferReportShowMaskToggle" @click="toggleMask" class="btn btn-sm btn-info d-flex align-items-center gap-2">
                                    <i :class="isMasked ? 'bi-eye-slash' : 'bi-eye'"></i>
                                    {{ isMasked ? "Unmask" : "Mask" }}
                                    </button>
                                    <div class="d-flex" v-if="menuAccess.transferReportShowExport">
                                        <div class="dropdown">
                ...
                <div class="col-xxl-12">
                    <div class="card mb-3">
                        <div class="card-header">
                            <h5 class="card-title">View Bulk & Single Transfer Report</h5>
                        </div>
                        <div class="card-body">
                            <!-- Success Alert -->
							<div
                                    ...
                                    <div class="mb-3 position-relative">
                                        ...
                                        <div class="d-flex justify-content-between align-items-center my-2 my-lg-0">
                                            <div v-if="headers">
                                                ...
                                                <br>
                                                <h6><span class="badge rounded-pill bg-success">Total Transfers</span></h6>
                                                <!-- Row start -->
