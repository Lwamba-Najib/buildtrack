...
const fetchDetails = async () => {
    ...
    try {
        ...
        const response = await axios.get(`/floatreportshow/${route.params.id}`, {
            headers: {
        ...
        if (response.status === 200 && response.data.success) {
            ...
            console.error('Error fetching float report:', response.statusText);
        }
    } catch (error) {
...
const fetchTotalCredits = async (mno = null) => {
    ...
    try {
        ...
        let url = `/floatreporttotalcredit/${route.params.id}`;  // No need to include clientId in the URL
        if (mno) {
            url += `/${mno}`;
        }
        ...
        if (response.status === 200 && response.data.success) {
            ...
            alerts.error = response.data.message || "Failed to load float totals.";
        }
    } catch (error) {
        handleError(error);
    } finally {
...
const exportPDF = async () => {
    ...
    try {
        ...
        const response = await axios.get(`/floatreportshowpdf/${route.params.id}`, {
            headers: {
        ...
        const filename = `${today}_floatreportshow_${id}.pdf`;
    ...
    <section>
        ...
        <div class="app-hero-header d-flex align-items-center">
            ...
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item">
                    <RouterLink to="/floatreportlist" class="text-decoration-none">Float Report</RouterLink>
                </li>
        ...
        <div class="app-body">
            ...
            <div class="row" 
                v-if="
					menuAccess.floatReportShowMaskToggle ||
					menuAccess.floatReportShowExport
				">
                        ...
                        <div class="card-body p-2">
                            <div class="d-flex justify-content-end my-1 my-lg-0">
                                <div class="d-flex flex-row gap-2">
                                    <button v-if="menuAccess.floatReportShowMaskToggle" @click="toggleMask" class="btn btn-sm btn-info d-flex align-items-center gap-2">
                                    <i :class="isMasked ? 'bi-eye-slash' : 'bi-eye'"></i>
                                    {{ isMasked ? "Unmask" : "Mask" }}
                                    </button>
                                    <div class="d-flex" v-if="menuAccess.floatReportShowExport">
                                        <div class="dropdown">
                ...
                <div class="col-xxl-12">
                    <div class="card mb-3">
                        <div class="card-header">
                            <h5 class="card-title">View Float Report</h5>
                        </div>
                        <div class="card-body">
                            <!-- Success Alert -->
							<div
                                    ...
                                    <div class="mb-3 position-relative">
                                        ...
                                        <div class="d-flex justify-content-between align-items-center my-2 my-lg-0">
                                            <div v-if="headers">
                                                ...
                                                <br>
                                                <h6><span class="badge rounded-pill bg-success">Total Float</span></h6>
                                                <!-- Row start -->
                                        ...
                                        <table id="table" class="table align-middle table-hover table-bordered m-0">
                                            ...
                                            <tbody>
                                                <template v-for="(summary, index) in transactions" :key="index">
                                                    <!-- Summary Date Row -->
                                                    <tr>
                                                        <td colspan="7" class="text-center fw-bold bg-light bg-opacity-1">{{ summary.date }}</td>
                                                    </tr>
                                                    <template v-for="(transaction, i) in summary.transactions" :key="i">
                                                        <!-- Normal Transactions -->
                                                        <tr>
                                                            <td>{{ transaction.date || '' }}</td>
                                                            <td>{{ maskValue(headers.client_number || 0, 'phone') }}</td>
                                                    ...
                                                    <tr class="bg-light fw-bold">
                                                        <td colspan="5" class="text-end">Totals for {{ summary.date }}:</td>
                                                        <td class="text-end">{{ maskValue(computeTotals(summary).amount.toLocaleString(),'') }}</td>
