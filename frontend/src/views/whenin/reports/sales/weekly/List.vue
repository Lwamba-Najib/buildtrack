...
const filterWeek = ref(""); // For week range picker
...
const fetchReportWeeklySales = async (page = 1) => {
	isLoading.value = true;
	try {
		const token = getToken();

		const response = await axios.get("/reportweeklysaleslist", {
			headers: {
		...
		if (response.status === 200) {
			...
			console.error("Error fetching weekly sales report:", response.statusText);
		}
	} catch (error) {
		handleError(error);
	} finally {
...
const exportFile = async (fileType) => {
	isLoading.value = true;
	const urls = {
		xlsx: "/reportweeklysalesxlsx",
		csv: "/reportweeklysalescsv",
	};
	...
	try {
		...
		const filename = `${today}_reportweeklysales.${fileType}`;
...
const initializeWeekPicker = () => {
	$(".weekpicker").daterangepicker(
		...
		function (start, end) {
			filterWeek.value = start.format("YYYY-MM-DD");
			$(".weekpicker").val(
	...
	<section>
		<div class="app-hero-header d-flex align-items-center">
			<ol class="breadcrumb">
				...
				<li class="breadcrumb-item">
					<RouterLink to="/reportweeklysaleslist" class="text-decoration-none">Sales</RouterLink>
				</li>
				<li class="breadcrumb-item text-secondary" aria-current="page">Weekly Report</li>
			</ol>
		</div>

		<div class="app-body">
			<div class="row" v-if="
				menuAccess.reportWeeklySalesFilter ||
				menuAccess.reportWeeklySalesExport
			">
						...
						<div class="card-body p-2">
							<div class="d-flex justify-content-end my-1 my-lg-0">
								<div class="d-flex flex-row gap-2">
									<button class="btn btn-sm btn-info" v-if="menuAccess.reportWeeklySalesFilter"
										@click="handleToggleFilterForms">
										<i class="fa fa-sliders"></i> Filter
									</button>
									<div class="d-flex" v-if="menuAccess.reportWeeklySalesExport">
										<div class="dropdown">
						...
						<div class="card-body">
							<div class="row gx-3">
								<div class="col-lg-12 col-sm-4 col-12">
									<div class="mb-3">
										<label for="filterWeek" class="form-label">Week Range</label>
										<div class="input-group">
											<input type="text" class="form-control weekpicker"
					...
					<div class="card mb-3">
						<div class="card-body">
							<strong class="d-flex align-items-center justify-content-between">
								Week Range
								<span class="text-default">{{ weekStart }} to {{ weekEnd }}</span>
							</strong>
							<hr>
							<strong class="d-flex align-items-center justify-content-between">
								Total Sales (UGX)
							...
							<div class="d-flex justify-content-between align-items-center my-2 my-lg-0">
								<div class="form-inline">
									<select name="paginationSize" class="form-select form-select-sm"
										v-model="paginationSize" @change="fetchReportWeeklySales">
						...
						<div class="card-body">
							...
							<div class="position-relative">
								...
								<div v-if="pagination.total > 0" class="d-flex justify-content-between mt-2">
									<div>
										{{
											`Showing ${pagination.currentPage > 1 ? (pagination.currentPage - 1) * paginationSize + 1 :
												1
											} to ${Math.min(
