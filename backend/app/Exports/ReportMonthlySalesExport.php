...
class ReportMonthlySalesExport implements FromCollection, WithHeadings, WithCustomStartCell, WithEvents
{
    ...
    public function __construct($selectedMonth = null)
    {
        // If no month provided, get the latest sales month
        if (!$selectedMonth) {
            $latestSale = Sales::orderBy('created_at', 'DESC')->first();
            $selectedMonth = $latestSale ? $latestSale->created_at->format('Y-m') : now()->format('Y-m');
        }

        $this->selectedMonth = $selectedMonth;
        $this->startDate = Carbon::parse($selectedMonth)->startOfMonth();
        $this->endDate = Carbon::parse($selectedMonth)->endOfMonth();
        ...
        return SalesItem::with([
            ...
            ->map(function ($item) {
                return [
                    ...
                    'created_at' => $item->created_at->format('Y-m-d') // Changed to match daily report
                ];
            });
    }
    ...
    public function headings(): array
    {
        return [
            ...
            'TOTAL AMOUNT',
            'ISSUED BY',
            'DATE', // Changed to match daily report
        ];
    }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function(AfterSheet $event) {
                ...
                $event->sheet->setCellValue('A1', 'Monthly Sales Report');
                $event->sheet->setCellValue('A2', 'Report Month: ' . $this->selectedMonth);
                $event->sheet->setCellValue('A3', 'Total Sales: ' . number_format($this->totalSales, 2));
                ...
                $event->sheet->getStyle('A1')->applyFromArray([
                    ...
                    'fill' => [
                        ...
                        'color' => ['rgb' => 'B22222'] // Changed to match daily report
                    ],
                    'alignment' => [
